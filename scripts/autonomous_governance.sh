...
if [ -n "${OPENCLAW_MEMORY_OS_GOVERNANCE_LOCK:-}" ]; then
  flock_path="$OPENCLAW_MEMORY_OS_GOVERNANCE_LOCK"
else
  default_lock_dir="${XDG_RUNTIME_DIR:-}"
  if [ -z "$default_lock_dir" ] && [ -n "${STATUS_FILE_PATH:-}" ]; then
    default_lock_dir="$(dirname "$STATUS_FILE_PATH")"
  fi
  if [ -z "$default_lock_dir" ] && [ -n "${XDG_STATE_HOME:-}" ]; then
    default_lock_dir="$XDG_STATE_HOME/openclaw-memory-os"
  fi
  if [ -z "$default_lock_dir" ]; then
    default_lock_dir="/tmp"
  fi
  mkdir -p "$default_lock_dir" 2>/dev/null || default_lock_dir="/tmp"
  flock_path="$default_lock_dir/openclaw-memory-os.governance.lock"
fi
exec 9>"$flock_path"
if ! flock -n 9; then
...
# Wave 2 (2026-07-21): capture scheduled_at + started_at so the
# dashboard can distinguish a true governance run from a hand-rolled
# invocation. ``GOVERNANCE_SCHEDULED_AT`` is normally provided by the
# systemd timer / caller; if not, we fall back to the script wall
# clock so the field is never None in the resulting status JSON.
GOVERNANCE_SCHEDULED_AT_VAL="${GOVERNANCE_SCHEDULED_AT:-$(date -u +%Y-%m-%dT%H:%M:%S.%6NZ || date -u +%Y-%m-%dT%H:%M:%SZ)}"
GOVERNANCE_STARTED_AT_VAL="$(date -u +%Y-%m-%dT%H:%M:%S.%6NZ || date -u +%Y-%m-%dT%H:%M:%SZ)"
export GOVERNANCE_SCHEDULED_AT="$GOVERNANCE_SCHEDULED_AT_VAL"
export GOVERNANCE_STARTED_AT="$GOVERNANCE_STARTED_AT_VAL"
...
set +e
# Wave 2 (2026-07-21): extended governance status protocol. The new
# fields are additive on top of the legacy 3-key schema. We only emit
# them when the caller (``scripts/_write_governance_status.py``) sees
# ``GOVERNANCE_STARTED_AT``; otherwise the writer falls back to the
# 3-key legacy shape so existing dashboard consumers keep working.
GOVERNANCE_FINISHED_AT_VAL="$(date -u +%Y-%m-%dT%H:%M:%S.%6NZ || date -u +%Y-%m-%dT%H:%M:%SZ)"
# Compute duration_seconds from started_at and finished_at via Python
# (the runner is a bash script, so we shell out once). ``date`` is too
# brittle for sub-second deltas; Python's ``datetime.fromisoformat``
# handles ISO-8601 with microsecond precision cleanly.
GOVERNANCE_DURATION_VAL="$(
  "$VENV_PY" - <<'PY' "$GOVERNANCE_STARTED_AT_VAL" "$GOVERNANCE_FINISHED_AT_VAL"
import sys
...
export GOVERNANCE_SCHEDULED_AT="$GOVERNANCE_SCHEDULED_AT_VAL"
export GOVERNANCE_STARTED_AT="$GOVERNANCE_STARTED_AT_VAL"
export GOVERNANCE_FINISHED_AT="$GOVERNANCE_FINISHED_AT_VAL"
export GOVERNANCE_DURATION="$GOVERNANCE_DURATION_VAL"
export GOVERNANCE_NEXT_SCHEDULED_AT="${GOVERNANCE_NEXT_SCHEDULED_AT:-}"
export GOVERNANCE_EXIT_CODE="${GOVERNANCE_EXIT_CODE:-$final_rc}"
export GOVERNANCE_MODE="${GOVERNANCE_MODE:-governance}"
