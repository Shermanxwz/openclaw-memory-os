...
Run this once on the operator's host after `sudo deploy/deploy.sh`. It is
the only gate intentionally left outside GitHub-hosted CI because it
requires the real systemd service, Qdrant corpus, Ollama models, session
database, network latency, and operator secrets.

This script is **not** a sustained-concurrency-5 certification gate. It
is a single-host, single-operator acceptance gate intended for the
personal / single-operator deployment profile described in `README.md`.
The performance gate inside this script is run with the
recommended interactive concurrency (1) only.
...
7. The varied-query performance gate passes at the recommended interactive
   concurrency (1). Sustained concurrency-5 low-latency operation is
   **not** part of this acceptance gate and is **not** certified by a
   successful run.
...
The script never invokes a physical-delete path. A successful run ends
by writing the evidence directory. Preserve that directory; it contains
the final source archive, built wheel, CPython 3.12 offline wheelhouse,
dependency locks, host proofs, and SHA-256 checksums.
