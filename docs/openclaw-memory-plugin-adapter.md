...
> **Independent community adapter. Not part of the OpenClaw project.**
> For typical keyword, vector, or hybrid search, prefer the OpenClaw
> built-in memory engine. This adapter is an optional community layer
> that lets an OpenClaw gateway load Memory OS as the selected memory
> runtime. It complements the built-in engine and does not replace it.
...
The adapter is **opt-in**. It only takes effect when an operator
explicitly installs the plugin and configures OpenClaw with
`plugins.slots.memory = "openclaw-memory-os"`. When the adapter is not
installed, or is installed but not selected, OpenClaw recall is
unaffected and the built-in memory engine handles the request on its
own. When `fallback=true` is set, Memory OS failures or empty results
fall back to the built-in engine. The adapter does not claim to save
tokens, and the bundled configuration is not assumed to be active in
any particular operator's production deployment.
