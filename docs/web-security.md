...
- `memory_os_session` — HttpOnly, Secure, SameSite=Strict
...
- Configurable lifetime: `MEMORY_OS_SESSION_MAX_AGE` (default 43200s
  = 12h)
...
- `csrf_token` cookie (non-HttpOnly, Secure, SameSite=Strict) issued
...
| ------------------- | :------: | :----: | :------: | -------------------------------------------- |
| `memory_os_session` |   Yes    |  Yes   |  Strict  | Auth session.                                |
| `csrf_token`        |    No    |  Yes   |  Strict  | CSRF token (must be JS-readable).            |
