...
> **This is an independent community project. It is not part of the
> OpenClaw project itself.** For typical keyword, vector, or hybrid
> search, use the OpenClaw built-in memory engine. Memory OS adds an
> optional layer for governance, evaluation, and observability. It
> **complements**, it does not replace.
...
The current release (v0.3.0-archived) is an archived source
distribution. The dashboard surface is read-only with respect to the
underlying memory corpus: it never writes back to the underlying
Qdrant collection or JSON file from the live dashboard. Lifecycle
operations (supersede, expire) are exposed through the maintenance
scripts under `scripts/` so operators retain explicit review before any
status changes touch the store.
