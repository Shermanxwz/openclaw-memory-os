...
def _ensure_csrf(request: Request, response: Response) -> str:
    ...
    if existing:
        return existing
    return issue_csrf_cookie(response, request=request)
...
def _build_app(settings: Settings | None = None) -> FastAPI:
    ...
    def _login_response(
        ...
        *,
        ...
        """Render login.html and always issue a matching CSRF cookie."""
        data = {
        ...
        if context:
            data.update(context)
        csrf = data.get("csrf_token") or request.cookies.get(CSRF_COOKIE_NAME) or generate_csrf_token()
        ...
        issue_csrf_cookie(resp, csrf, request=request)
        return resp
    ...
    app = FastAPI(
        ...
        version="0.3.0",
        description=(
            "Privacy-clean dashboard and recall-testing layer for OpenClaw-style "
    ...
    def login(
        ...
        set_session_cookie(response, session_token, settings, request=request)
        issue_csrf_cookie(response, request=request)
        return response
