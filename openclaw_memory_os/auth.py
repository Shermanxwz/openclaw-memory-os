...
1. **Bearer / cookie token** (``MEMORY_OS_TOKEN``) — the historical,
   ...
   responds with an HttpOnly + Secure + SameSite=Lax session cookie.
...
def _request_is_secure(request: Optional[Request] = None) -> bool:
    """Return whether cookies for this request should use the Secure flag.

    Public dashboard traffic is terminated by nginx and proxied to uvicorn.
    With uvicorn --proxy-headers enabled, request.url.scheme reflects
    X-Forwarded-Proto and stays https for the public origin. Direct local/LAN
    HTTP access must not receive Secure cookies, otherwise browsers drop the
    CSRF/session cookie and login POST fails on new devices.

    If no request is available, fail secure to preserve the historical default.
    """
    if request is None:
        return True
...
def issue_csrf_cookie(response: Response, token: Optional[str] = None, *, request: Optional[Request] = None) -> str:
