...
(function (global) {
  ...
  function renderHistory(node, history) {
    ...
    const rows = (history || []).map(function (h) {
      return '<tr>' +
        '<td>' + OC.escapeHTML(h.run_id || h.query_id || h.id || "—") + '</td>' +
        '<td>' + OC.escapeHTML(h.mode || "—") + '</td>' +
        '<td>' + OC.escapeHTML(h.policy_version || "—") + '</td>' +
        '<td>' + OC.escapeHTML(String(h.useful_total || 0)) + ' / ' + OC.escapeHTML(String(h.feedback_total || 0)) + '</td>' +
        '<td>' + OC.escapeHTML(h.finished_at || h.evaluated_at || "—") + '</td>' +
      '</tr>';
    });
    if (!rows.length) {
    ...
    node.innerHTML = '<table><thead><tr>' +
      '<th>Run ID</th><th>Mode</th><th>Policy</th><th>有用 / 总反馈</th><th>时间</th>' +
