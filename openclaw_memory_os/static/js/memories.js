...
(function (global) {
  ...
  function renderCollection(node, name, items) {
    ...
    const table = '<table><thead><tr>' +
      '<th>ID</th><th>层级</th><th>状态</th><th>Collection</th><th>内容(预览)</th><th>更新时间</th>' +
  ...
  function render() {
    ...
    OC.getJSON("/api/dashboard/memories?limit=200")
      .then(function (payload) {
        ...
        const totalCount = (payload && payload.count) || memories.length;
        if (meta) {
          meta.innerHTML = '' +
            '<div class="grid">' +
              ...
              '<div class="card"><h2>全库记忆数</h2><div class="stat">' + OC.escapeHTML(String(totalCount)) + '</div></div>' +
              '<div class="card"><h2>本页显示</h2><div class="stat">' + OC.escapeHTML(String(memories.length)) + '</div></div>' +
