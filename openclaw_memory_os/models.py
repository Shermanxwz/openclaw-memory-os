...
class AutonomousGovernanceJob(BaseModel):
    ...
    **when it will run next**, and **what the result was**, with the schedule,
    ...
    # Wave 2 (2026-07-21): extended governance protocol fields. All are
    # ``None`` by default so legacy callers keep working. They are
    # populated from ``_read_autonomous_governance_status`` when the
    # runner exports ``GOVERNANCE_STARTED_AT``/``GOVERNANCE_FINISHED_AT``
    # etc. via the systemd timer path.
    scheduled_at: Optional[str] = Field(
        default=None,
        description="ISO-8601 timestamp the systemd timer asked the job to start.",
    )
    started_at: Optional[str] = Field(
        default=None,
        description="ISO-8601 timestamp the runner actually began executing.",
    )
    finished_at: Optional[str] = Field(
        default=None,
        description="ISO-8601 timestamp the runner finished executing.",
    )
    duration_seconds: Optional[int] = Field(
        default=None,
        description="Wall-clock duration in seconds (finished_at - started_at).",
    )
    next_scheduled_at: Optional[str] = Field(
        default=None,
        description=(
            "ISO-8601 timestamp the systemd timer schedules for the next run. "
            "Distinct from ``next_run`` (which is computed deterministically "
            "from the fixed contract); ``next_scheduled_at`` is the value "
            "the timer actually exposes (NextElapseUSecRealtime)."
        ),
    )
    exit_code: Optional[int] = Field(
        default=None,
        description="Process exit code from the most recent governance run.",
    )
    exit_code: Optional[int] = Field(
        default=None,
        description="Process exit code from the most recent governance run.",
    ...
    runner_mode: Optional[str] = Field(
        default=None,
        description=(
            "Mode label the runner emitted (e.g. ``governance`` for the "
            "weekly deep-audit path, ``maintenance`` for the daily path). "
            "Distinct from the contract ``mode`` field above, which is "
            "always ``FORCE_CONTENT_SUPERSEDE=1`` for the weekly job."
        ),
    )
    ...
    def for_dashboard(
        ...
        *,
        ...
        Wave 2 (2026-07-21): the runner now optionally emits the full
        extended protocol (``scheduled_at`` / ``started_at`` / ``finished_at``
        / ``duration_seconds`` / ``next_scheduled_at`` / ``exit_code`` /
        ``mode``). All are passed through verbatim here when provided.
        ``runner_mode`` is named to avoid clashing with the contract
        ``mode`` field (always ``FORCE_CONTENT_SUPERSEDE=1``).
        """
...
class MaintenanceHealth(BaseModel):
    ...
    # Wave 2 (2026-07-21): ISO-8601 UTC timestamp of the next scheduled
    # maintenance run, derived from the live
    # ``openclaw-memory-os-maintenance.timer`` unit when no explicit
    # value is available in the maintenance-summary JSON. The dashboard
    # treats ``None`` and missing as "unknown" so operators can tell a
    # genuinely-unconfigured box from one where the timer was just
    # momentarily unparsable.
    next_scheduled_at: Optional[str] = Field(
        default=None,
        description="ISO-8601 UTC timestamp of the next scheduled maintenance run.",
    )
...
class LastMaintenanceSummary(BaseModel):
    ...
    The fields below are populated from the JSON written by
    ``scripts/_write_summary.py`` (which reads ``MAINTENANCE_*`` env vars
    injected by ``scripts/maintenance.sh``).

    Status semantics:
      - ``status`` ∈ {None, "success", "failed", "ok"} (None = legacy/unknown)
      - ``exit_code`` = 0 for success, 1 for failure
      - ``failed_step`` = name of first failing step (None on success)
      - ``last_success_at`` = finished_at of last run with status="success"
        (does NOT regress on failure; carries forward previous success)
      - ``started_at`` / ``finished_at`` = ISO-8601 timestamps for THIS run
    """
    ...
    # Status fields (v0.3.x). Older summaries without these will parse to None.
    started_at: Optional[str] = Field(default=None, description="ISO-8601 UTC when maintenance.sh was invoked.")
    finished_at: Optional[str] = Field(default=None, description="ISO-8601 UTC when maintenance.sh exited.")
    status: Optional[str] = Field(default=None, description='"success" or "failed" or legacy "ok".')
    exit_code: Optional[int] = Field(default=None, description="Process exit code: 0 = success, 1 = failure.")
    failed_step: Optional[str] = Field(default=None, description="Name of the first failing step, or None on success.")
    last_success_at: Optional[str] = Field(default=None, description="ISO-8601 UTC of last run that exited 0 (does NOT regress on failure).")
...
class HealthSummary(BaseModel):
    ...
    # Wave 2 (2026-07-21): systemd-timer schedule, read live by
    # analytics._read_systemd_timer_schedule() and surfaced on the
    # dashboard so the schedule card never has to hardcode
    # ``Tue 04:01`` / ``daily 07:45``. Both dicts share the same
    # shape: ``calendar`` (OnCalendar expression), ``last_trigger``
    # (ISO-8601 UTC), ``next_elapse`` (ISO-8601 UTC), ``active_state``,
    # ``result``. Missing timers come back as ``{"active_state": "unknown"}``.
    maintenance_schedule: Dict[str, Any] = Field(
        default_factory=dict,
        description="Systemd timer schedule for maintenance (openclaw-memory-os-maintenance.timer).",
    )
    governance_schedule: Dict[str, Any] = Field(
        default_factory=dict,
        description="Systemd timer schedule for autonomous governance (openclaw-memory-os-governance.timer).",
    )
