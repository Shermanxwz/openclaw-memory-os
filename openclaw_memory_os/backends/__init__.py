    ...
    * Vector search for dense recall is delegated to Qdrant via
    ...
    def _embed(self, text: str) -> List[float]:
        ...
        EmbeddingUnavailable
            ...
            real ranking. The fallback path that used to return
            ``[0.0] * 768`` has been removed (B2-6); callers that
            legitimately want the zero vector (e.g. integration
            tests that want to confirm the strict dense path raises)
            should mock ``_embed`` directly.
        """
        import os
        ...
        prompt = text[:4000]
        cache_key = (ollama_url, model, prompt)
        ...
        try:
            ...
            raise EmbeddingUnavailable(
                f"embedding service unreachable ({ollama_url}, model={model}): {exc}"
            ) from exc
        vec = data.get("embedding") if isinstance(data, dict) else None
        if not vec:
            raise EmbeddingUnavailable(
                f"embedding service returned no vector "
                f"(ollama_url={ollama_url}, model={model}); "
                "refusing to send a zero vector to Qdrant "
                f"(hard contract: {NO_ZERO_VECTOR_FAKE_SUCCESS})."
            )
        # B2-6: a zero / all-NaN vector is just as bad as an
        # empty one. Qdrant would happily accept it and return
        # whichever point happened to be closest in numeric
        # terms, which is not a real ranking. Reject it.
        try:
            if any(float(x) != 0.0 for x in vec) is False:
                raise EmbeddingUnavailable(
            ...
            raise EmbeddingUnavailable(
                f"embedding service returned a non-numeric vector: {exc}"
            ) from exc
