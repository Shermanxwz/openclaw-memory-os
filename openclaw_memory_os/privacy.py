...
def _is_gitignored(path: Path) -> bool:
    """Return True when git would ignore ``path`` in the current checkout."""
    try:
        rel = path if not path.is_absolute() else path.relative_to(Path.cwd())
    except ValueError:
        rel = path
    try:
...
def scan_paths(
    ...
    *,
    ...
    Args:
        ...
        include_ignored: When False (default), skip files ignored by git so
            local audit archives, `.env`, `.secrets`, build outputs, and
            other non-release artifacts do not fail the public-release gate.
    """
    ...
    for path in _iter_files(paths):
        rel = str(path)
        if not include_ignored and _is_gitignored(path):
            continue
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--include-ignored",
        action="store_true",
        help="Also scan files ignored by git (local audit archives, .env, .secrets, build outputs).",
    )
    args = parser.parse_args(list(argv) if argv is not None else None)

    findings = scan_paths(args.paths, baseline=args.baseline, include_ignored=args.include_ignored)
