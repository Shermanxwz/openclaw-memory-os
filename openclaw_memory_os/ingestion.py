...
# Provider configuration is read lazily through
# :func:`openclaw_memory_os.embed_provider.get_embed_provider`. The
# wave-2 contract is: when ``EMBED_PROVIDER`` is unset, the provider
# falls back to the OVH-local Ollama (``nomic-embed-text``); setting
# ``EMBED_PROVIDER=newapi`` routes to NewAPI via the OpenAI-compatible
# ``/v1/embeddings`` endpoint with the real model name
# ``qwen3-embedding:0.6b`` (or whatever ``EMBED_PROVIDER_MODEL`` is set
# to). Default dim is 768 to match the existing Qdrant collections.
EMBED_DIM = 768

# QDRANT_URL and QDRANT_API_KEY are resolved lazily through
# :func:`_resolve_qdrant_settings` so we honour the same env contract as
# :mod:`openclaw_memory_os.config` (QDRANT_URL, QDRANT_API_KEY) without
# duplicating config state. The default matches the documented project
# default in ``.env.example``; the resolver never raises and never logs
# the secret.

_QDRANT_URL_CACHE: Optional[str] = None
_QDRANT_API_KEY_CACHE: Optional[str] = None


def _resolve_qdrant_settings() -> tuple[str, Optional[str]]:
    """Return ``(url, api_key)`` for the local Qdrant endpoint.

    Reads ``QDRANT_URL`` and ``QDRANT_API_KEY`` from the process env. The
    URL is cached on first read because the static helpers below may be
    invoked many times in one process.
    """
    global _QDRANT_URL_CACHE, _QDRANT_API_KEY_CACHE
    if _QDRANT_URL_CACHE is None:
        _QDRANT_URL_CACHE = (
            os.environ.get("QDRANT_URL") or "http://127.0.0.1:6333"
        ).rstrip("/")
        _QDRANT_API_KEY_CACHE = os.environ.get("QDRANT_API_KEY") or None
    return _QDRANT_URL_CACHE, _QDRANT_API_KEY_CACHE


def _qdrant_request_headers() -> dict[str, str]:
    """Return JSON headers for outbound Qdrant calls (incl. api-key auth)."""
    _, api_key = _resolve_qdrant_settings()
    headers = {"Content-Type": "application/json"}
    if api_key:
        # Qdrant expects the ``api-key`` header (not ``Authorization``) when
        # using its native auth scheme; match the python client's contract.
        headers["api-key"] = api_key
    return headers


def _qdrant_url(path: str) -> str:
    """Build an absolute Qdrant URL from ``path`` (must start with ``/``)."""
    base, _ = _resolve_qdrant_settings()
    return f"{base}{path}"
...
class IngestionManager:
    ...
    def _embed(self, text: str) -> Optional[List[float]]:
        """Embed a chunk via the configured provider.

        Wave-2 (2026-07-20): delegates to
        :mod:`openclaw_memory_os.embed_provider`. Behaviour is
        unchanged for the default ``ollama`` provider (POST
        ``/api/embeddings`` with ``model=nomic-embed-text``). When
        ``EMBED_PROVIDER=newapi``, calls NewAPI's
        ``/v1/embeddings`` with the real model name
        ``qwen3-embedding:0.6b`` and ``dimensions=768`` so the
        returned vector is the same 768-dim shape the existing
        Qdrant collections expect.

        Returns ``None`` (legacy contract) on any embed failure so
        the caller can record ``failed++`` and skip the chunk. The
        provider itself raises :class:`EmbeddingUnavailable` /
        :class:`EmbeddingDimensionMismatch`; both are caught here
        and converted to ``None`` so a transient NewAPI outage does
        not abort the whole ingestion run. The next provider call
        (next chunk) re-tries — that is the pre-wave-2 retry
        semantics this method has always had.
        """
        try:
            provider = get_embed_provider()
        except Exception as exc:
            logger.error("Embed provider init failed: %s", exc)
            return None
        try:
            return provider.embed(text)
        except EmbeddingDimensionMismatch as exc:
            logger.error(
                "Embedding dim mismatch (provider=%s, model=%s): %s",
                provider.name,
                provider.model,
                exc,
            )
            ...
            # Includes network errors, malformed payloads, and
            # degenerate-vector rejections. Preserve the pre-wave-2
            # contract: log and return None so the caller treats it
            # as a retryable failure rather than crashing the run.
            logger.error("Embedding unavailable (provider=%s): %s", provider.name, exc)
            return None
        except Exception as exc:
            logger.error("Embed error (provider=%s): %s", provider.name, exc)
            return None
    ...
    def _fetch_existing_ids(self, collection: str) -> Set[str]:
        ...
        try:
            offset = None
            url = _qdrant_url(f"/collections/{collection}/points/scroll")
            while True:
    ...
    def _qdrant_collection_exists(name: str) -> bool:
        try:
            with urllib.request.urlopen(_qdrant_url(f"/collections/{name}"), timeout=10) as r:
                return r.status == 200
        except Exception:
            return False
    ...
    def _qdrant_upsert(name: str, points: List[dict]) -> None:
        ...
        body = {"points": points}
        req = urllib.request.Request(
            _qdrant_url(f"/collections/{name}/points?wait=true"),
            data=json.dumps(body).encode("utf-8"),
            headers=_qdrant_request_headers(),
            method="PUT",
        )
    ...
    def _build_payload(self, text: str, source: Path, chunk_id: str, taxonomy=None) -> dict:
        ...
        # Record the active embed provider + model so the dashboard
        # can tell which model produced a given point. ``provider``
        # is one of ``"ollama"`` / ``"newapi"``; ``model`` is the
        # real model name (e.g. ``nomic-embed-text``,
        # ``qwen3-embedding:0.6b``). We read the provider here
        # without caching so a mid-run env switch (rare, but
        # supported) is reflected in the payload.
        try:
            _prov = get_embed_provider()
            _prov_name = _prov.name
            _prov_model = _prov.model
        except Exception:
            _prov_name = "ollama"
            _prov_model = os.environ.get("EMBED_MODEL", "nomic-embed-text")
        ...
        return {
            ...
            "tags": [stem],
            "embedding_model": _prov_model,
            "embedding_provider": _prov_name,
