...
def test_qdrant_embedding_lru_uses_one_http_call(monkeypatch):
    ...
    def fake_post(*args, **kwargs):
        calls["count"] += 1
        return Response()
    # Wave 2 (2026-07-20): ``_embed`` delegates to
    # :mod:`openclaw_memory_os.embed_provider`, which uses ``httpx``.
    # Stub the lazy httpx client so the LRU cache contract is still
    # exercised without standing up a real gateway.
    from openclaw_memory_os import embed_provider as _ep
    _ep.reset_provider_caches()
    monkeypatch.setenv("EMBED_PROVIDER", "ollama")
    monkeypatch.setenv("EMBED_PROVIDER_DIM", "2")
    provider = _ep.get_embed_provider()
    provider.expected_dim = 2
