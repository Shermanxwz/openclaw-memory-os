...
def test_governance_js_uses_exclusion_wording_for_bulk_button():
    src = _js("governance.js")
    # The bulk button label must talk about "exclude" or "review"
    # — never "delete".
    assert "排除" in src or "review-only" in src.lower()
    # The bulk action path goes to consolidation/duplicates — never
    # to a delete endpoint.
    assert "/api/consolidate-duplicates" in src
