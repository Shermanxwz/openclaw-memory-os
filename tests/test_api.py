...
def test_auth_required_when_token_set(monkeypatch, tmp_path):
    ...
    with TestClient(app) as c:
        ...
        # API rejects wrong query-string tokens when no independent bearer/cookie
        # auth is present. Use a fresh client so the session cookie issued above
        # does not correctly authenticate the request.
        with TestClient(app) as unauthenticated:
            api4 = unauthenticated.get("/api/health?token=wrong")
