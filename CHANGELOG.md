...
- **Unified RetrievalEngine** (`openclaw_memory_os/retrieval_engine.py`). Single
  ...
  only with graceful degradation to lexical on embedding failure), and `hybrid`
  (Weighted RRF + feature rerank). Mode-specific semantics are enforced inside
  the engine and shared by API and CLI.
...
- `ranking.py` is retained for backward compatibility but all production recall
  paths should route through `RetrievalEngine.retrieve()`.
...
- **475 tests pass** (v0.2.16 baseline 291 + 184 new).
- `bash scripts/privacy_scan.sh` — **0 findings**.
- `bash -n` clean on all shell scripts.
- Manual run of the full autonomous_governance.sh on the host confirms the
  evolution lock, feedback replay, deep audit, and status write paths.
